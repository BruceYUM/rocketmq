...
public class DefaultMQProducerImpl implements MQProducerInner {
     ...
     */
    public void start(final boolean startFactory /*MARK 该方法调用处入参都为 true*/) throws MQClientException {
    ...
    public void checkTransactionState(final String addr, final MessageExt msg,
        ...
        Runnable request = new Runnable() {
            ...
            public void run() {
                ...
                if (transactionCheckListener != null) {
                    ...
                    try {
                        // 终调用应用程序实现的TransactionListener的checkLocalTransaction方法，返回事务状态
            ...
            private void processTransactionState(
                ...
                switch (localTransactionState) {
                    case COMMIT_MESSAGE:
                        // 如果事务状态为Local-Transaction State#COMMIT_MESSAGE，则向消息服务器发送提交事务消息命令.
        ...
        // 任务提交到TransactionMQProducer的线程池中执行
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            ...
            //本地没有缓存topic的路由信息，查询NameServer尝试获取
    ...
    private SendResult sendKernelImpl(final Message msg,// 待发送消息。
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                // MARK 如果是事务Prepared消息，则设置消息的系统标记为MessageSysFlag.TRANSACTION_PREPARED_TYPE
                ...
                // 建消息发送请求包。主要包含如下重要信息：生产者组、主题名称、默认创建主题Key、该主题在单个Broker默认队列数、队列ID（队列序号）、
     ...
     * KERNEL SYNC -------------------------------------------------------
     * 同步向指定队列发送消息。
     * 同步向指定队列发送消息时，如果只有一个发送线程，在发送到某个指定队列中时，
     * 这个指定队列中的消息是有顺序的，那么就按照发送时间排序；
     * 如果某个Topic的队列都是这种情况，那么我们称该Topic的全部消息是分区有序的。
     */
    ...
    /**
     * DEFAULT SYNC -------------------------------------------------------
     * 同步发送普通消息。设置的超时时间可以通过sendMsgTimeout进行变更，其默认值为3s。
     */
     ...
     * 其关键就是根据本地执行事务的状态分别发送提交、回滚或“不作为”的命令。
